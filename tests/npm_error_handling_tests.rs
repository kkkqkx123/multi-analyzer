...
fn create_broken_lint_project() -> PathBuf {
    ...
    fs::write(temp_dir.join("package.json"), package_json).expect("Failed to write package.json");
...
fn create_lint_with_errors_project() -> PathBuf {
    ...
    fs::write(temp_dir.join("package.json"), package_json).expect("Failed to write package.json");
    fs::write(temp_dir.join("src/index.js"), js_file).expect("Failed to write index.js");
...
fn test_npm_analyzer_command_failure_no_issues() {
    ...
    assert!(
        result.is_ok(),
        "Analyzer should not fail even when command fails"
    );
...
"#;
    ...
    // Verify file paths
    let file_paths: Vec<&str> = issues
        .iter()
        .map(|i| i.location.file_path.as_str())
        .collect();
    assert!(
        file_paths.contains(&"src/index.ts"),
        "Should contain src/index.ts"
    );
    assert!(
        file_paths.contains(&"src/utils.ts"),
        "Should contain src/utils.ts"
    );
...
"#;
    ...
    assert!(!issues.is_empty(), "Should parse npm audit errors");
...
fn test_pnpm_analyzer_execution() {
    ...
    assert!(
        result.is_ok(),
        "Pnpm analyzer should complete without error"
    );
}
...
"#;
    ...
    assert_eq!(
        issues.len(),
        6,
        "Should parse 6 issues from ESLint verbose output"
    );

    // Verify first issue details
    let first_issue = &issues[0];
    assert_eq!(
        first_issue.location.file_path,
        "D:\\项目\\agent\\graph-agent\\apps\\cli-app\\src\\commands\\agent\\index.ts"
    );
    assert_eq!(first_issue.location.line_number, Some(13));
    assert_eq!(first_issue.location.column_number, Some(31));
    assert!(matches!(
        first_issue.level,
        analyzer::core::IssueLevel::Warning
    ));
    assert!(first_issue
        .message
        .contains("parseAndValidateAgentLoopConfig"));

    // Verify second file's issue
    let last_issue = &issues[5];
    assert_eq!(
        last_issue.location.file_path,
        "D:\\项目\\agent\\graph-agent\\apps\\cli-app\\src\\commands\\agent\\utils.ts"
    );
    assert_eq!(last_issue.location.line_number, Some(10));
    assert!(matches!(
        last_issue.level,
        analyzer::core::IssueLevel::Error
    ));
...
"#;
    ...
    assert!(
        !first_msg.contains("@typescript-eslint"),
        "Rule name should be extracted from message"
    );

    let second_msg = &issues[1].message;
    assert!(second_msg.contains("Unexpected any"));
    assert!(
        !second_msg.contains("@typescript-eslint"),
        "Rule name should be extracted from message"
    );
}
